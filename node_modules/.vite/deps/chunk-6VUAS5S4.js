...
var G = class extends u {
  ...
  async applyEdits(e2, t3) {
    ...
    const { layer: a2 } = this;
    await H2(a2, "editing");
    const n = "infoFor3D" in a2 ? a2.infoFor3D : null, o3 = null != n, u3 = o3 || (t3?.globalIdUsed ?? false), l2 = o3 ? await this._uploadMeshesAndGetAssetMapEditsJSON(e2) : null, c2 = e2.addFeatures?.map((e3) => m3(this.layer, e3, n)) ?? [], d = (await Promise.all(c2)).filter(N), p = e2.updateFeatures?.map((e3) => m3(this.layer, e3, n)) ?? [], y2 = (await Promise.all(p)).filter(N), h2 = b2(this.layer, e2.deleteFeatures, u3);
    i3(d, y2, a2.spatialReference);
    const m4 = await f3(this.layer, e2), f4 = a2.capabilities.editing.supportsAsyncApplyEdits && o3, g2 = t3?.gdbVersion || a2.gdbVersion, q = { gdbVersion: g2, rollbackOnFailure: t3?.rollbackOnFailureEnabled, useGlobalIds: u3, returnEditMoment: t3?.returnEditMoment, usePreviousEditMoment: t3?.usePreviousEditMoment, async: f4 };
    await i4(this.layer.url, g2, true);
    const w = c(this.layer.url, g2 || null);
    if (await o2(a2.url, g2, a2.historicMoment)) throw new s2("feature-layer-source:historic-version", "Editing a historic version is not allowed");
    t3?.returnServiceEditsOption ? (q.edits = JSON.stringify([{ id: a2.layerId, adds: d.length ? d : null, updates: y2.length ? y2 : null, deletes: h2.length ? h2 : null, attachments: m4, assetMaps: l2 }]), q.returnServiceEditsOption = V2.toJSON(t3?.returnServiceEditsOption), q.returnServiceEditsInSourceSR = t3?.returnServiceEditsInSourceSR) : (q.adds = d.length ? JSON.stringify(d) : null, q.updates = y2.length ? JSON.stringify(y2) : null, q.deletes = h2.length ? u3 ? JSON.stringify(h2) : h2.join(",") : null, q.attachments = m4 && JSON.stringify(m4), q.assetMaps = null != l2 ? JSON.stringify(l2) : void 0);
  ...
  async uploadAssets(e2, t3) {
    const { uploadAssets: r2 } = await import("./uploadAssets-YW25ZFU5.js");
  ...
  async _uploadMeshesAndGetAssetMapEditsJSON(e2) {
    ...
    for (const i5 of t3) {
      ...
      if (t(t4)) s4.push(e3);
        ...
        const t5 = e3.origin, { convertMeshVertexSpace: r3 } = await import("./convertMeshVertexSpace-VVP75G5E.js"), n = await r3(e3, new i2({ origin: [t5.x, t5.y, t5.z ?? 0] }));
  ...
  _createEditsResult(e2) {
    ...
    if (s4.length > 0) {
      ...
      for (const e3 of s4) {
        ...
        i5.editedFeatureResults.push({ layerId: e3.id, editedFeatures: I(t4, r3) });
      }
    }
    return i5;
  }
  ...
  async _areAllAssetsAlreadyMapped(e2) {
    ...
    const { assetMapFieldRoles: { parentGlobalId: c2, assetHash: d } } = i5, p = new b({ where: `${c2} IN (${l2.map((e3) => `'${e3}'`)})`, outFields: [d, c2], returnGeometry: false }), y2 = await s3(o3, p), { features: m4 } = y2;
...
//# sourceMappingURL=chunk-6VUAS5S4.js.map
