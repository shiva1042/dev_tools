...
function o3(e4, r2) {
  ...
  if (U2 && (I.sr = A(U2)), I.time = b2 ? [b2.start, b2.end].join(",") : null, y2) {
    const { xmin: e5, ymin: t, xmax: r3, ymax: i2 } = y2;
    I.mapExtent = `${e5},${t},${r3},${i2}`;
  }
  return j && (I.layerDefs = j), S2 && !j && (I.dynamicLayers = S2), I.layers = "popup" === f5 ? "visible" : f5, N2 && !S2 && (I.layers += `:${N2.join(",")}`), I;
}
function l3(e4) {
  const { mapExtent: t, floors: i2, width: o4, sublayers: l4, layerIds: m7, layerOption: p3, gdbVersion: f5 } = e4, y2 = l4?.find((e5) => null != e5.layer)?.layer?.serviceSublayers, u7 = "popup" === p3, c2 = {}, d2 = i({ extent: t, width: o4, spatialReference: t?.spatialReference }), g = [], x = (e5) => {
    const t2 = 0 === d2, r2 = 0 === e5.minScale || d2 <= e5.minScale, i3 = 0 === e5.maxScale || d2 >= e5.maxScale;
    if (e5.visible && (t2 || r2 && i3)) if (e5.sublayers) e5.sublayers.forEach(x);
    else {
...
function a3(t, r2) {
  const i2 = !!t?.length, s4 = r2.filter((e4) => null != e4.definitionExpression || i2 && null != e4.floorInfo);
  return s4.length ? s4.map((r3) => {
    const i3 = n3(t, r3), s5 = l(i3, r3.definitionExpression);
...
async function f4(u7, i2, f5) {
  const c2 = (i2 = a4(i2)).geometry ? [i2.geometry] : [], l4 = f3(u7);
  return l4.path += "/identify", P(c2).then((e4) => {
    const t = o3(i2, { geometry: e4?.[0] }), u8 = u3({ ...l4.query, f: "json", ...t }), a5 = s3(u8, f5);
    return H(l4.path, a5).then(m6).then((r2) => p2(r2, i2.sublayers));
  });
}
function m6(r2) {
  const e4 = r2.data;
...
var P2 = class extends b {
  ...
  _updatePointSymbology(e4) {
    const r2 = e4.sourceLayer && "renderer" in e4.sourceLayer && e4.sourceLayer.renderer, { highlightGraphicUpdated: i2, highlightGraphics: s4, layerView: { view: o4 } } = this, a5 = (e5) => {
      e5.visible || (e5.visible = true, i2?.({ graphic: e5, property: "visible", oldValue: false, newValue: true }));
    };
    r2 && "getSymbolAsync" in r2 ? r2.getSymbolAsync(e4).then(async (i3) => {
      i3 ||= new u2();
      let l4 = null;
      const n4 = "visualVariables" in r2 ? r2.visualVariables?.find((e5) => "size" === e5.type) : void 0;
      n4 && (G || (G = (await import("./visualVariableUtils-C4JZK66N.js")).getSize), l4 = G(n4, e4, { view: o4.type, scale: o4.scale, shape: "simple-marker" === i3.type ? i3.style : null })), l4 ||= "width" in i3 && "height" in i3 && null != i3.width && null != i3.height ? Math.max(i3.width, i3.height) : "size" in i3 ? i3.size : 16, s4?.includes(e4) && (this._setGraphicSymbol(e4, new u2({ style: "square", size: l4, color: new u([255, 255, 255, 1 / 255]), xoffset: "xoffset" in i3 ? i3.xoffset : 0, yoffset: "yoffset" in i3 ? i3.yoffset : 0 })), a5(e4));
    }) : a5(e4);
  }
  get _updateContext() {
  ...
  async _updateHighlightedFeaturesGeometries(e4) {
    ...
    if (!t) return;
    const r2 = this._getTargetResolution(e4), i2 = /* @__PURE__ */ new Map(), { highlightGraphics: s4, highlightGraphicUpdated: a5 } = t;
    for (const c2 of s4) if (!this._featuresResolutions.has(c2) || this._featuresResolutions.get(c2) > r2) {
      const e5 = c2.sourceLayer;
      e(i2, e5, () => /* @__PURE__ */ new Map()).set(c2.getObjectId(), c2);
    }
    const { layerView: { view: l4 } } = this, n4 = Array.from(i2, ([e5, t2]) => {
      const i3 = e5.createQuery();
      return i3.objectIds = [...t2.keys()], i3.outFields = [e5.objectIdField], i3.returnGeometry = true, i3.maxAllowableOffset = r2, i3.outSpatialReference = l4.spatialReference, e5.queryFeatures(i3);
    }), p3 = await Promise.all(n4);
    if (!this.destroyed) for (const { features: o4 } of p3) for (const e5 of o4) {
      const t2 = e5.sourceLayer, o5 = i2.get(t2).get(e5.getObjectId());
  ...
  async _fetchPopupFeaturesUsingIdentify(e4, t, r2) {
    const i2 = await this._createIdentifyParameters(e4, t, r2);
    if (null == i2) return [];
    const { results: s4 } = await f4(this.layerView.layer.parsedUrl, i2, r2);
    return s4.map((e5) => e5.feature);
  }
  async _createIdentifyParameters(e4, t, r2) {
    const { floors: i2, layer: s4, timeExtent: o4, view: { spatialReference: a5, scale: l4 } } = this.layerView;
    ...
    return new c({ floors: i2, gdbVersion: "gdbVersion" in s4 ? s4.gdbVersion : void 0, geometry: e4, height: u7, layerOption: "popup", mapExtent: h, returnGeometry: true, spatialReference: a5, sublayers: s4.sublayers, timeExtent: o4, tolerance: n4, width: u7 });
  }
  async _fetchPopupFeaturesUsingQueries(e4, t, r2) {
    const { layerView: { floors: s4, timeExtent: o4 } } = this, a5 = t.map(async ({ sublayer: t2, popupTemplate: i2 }) => {
      if (await t2.load(r2).catch(() => {
      }), t2.capabilities && !t2.capabilities.operations.supportsQuery) return [];
      const a6 = t2.createQuery(), n4 = o2({ renderer: t2.renderer, pointerType: r2?.pointerType }), p3 = this.createFetchPopupFeaturesQueryGeometry(e4, n4), u7 = /* @__PURE__ */ new Set(), [h] = await Promise.all([p(t2, i2), t2.renderer?.collectRequiredFields(u7, t2.fieldsIndex)]);
      ...
      if (a6.where = l(a6.where, d2), t2.capabilities?.query.supportsOrderBy && t2.orderBy?.[0]) {
        const e5 = t2.orderBy[0], r3 = !e5.valueExpression && e5.field, i3 = "ascending" === e5.order ? "asc" : "desc";
        r3 && (a6.orderByFields = [`${r3} ${i3}`]);
      }
      const m7 = this._getTargetResolution(p3.width / n4), w2 = await V(i2);
      s2(r2);
      const v2 = "point" === t2.geometryType || w2 && w2.arcadeUtils.hasGeometryOperations(i2);
...
async function A2(e4, t, r2) {
  ...
  return i2 && "defaultSymbol" in i2 && !i2.defaultSymbol && (t = i2.valueExpression ? await Promise.all(t.map((e5) => i2.getSymbolAsync(e5, r2).then((t2) => t2 ? e5 : null))).then((e5) => e5.filter((e6) => null != e6)) : t.filter((e5) => null != i2.getSymbol(e5))), t;
}
...
//# sourceMappingURL=chunk-LLI23MNT.js.map
