...
async function m(e, t, r) {
  const { geometry: s2 } = t, i2 = { ...t.attributes };
  if (null != r && "mesh" === s2?.type) {
    const { transformFieldRoles: t2 } = r, { origin: c, spatialReference: p, vertexSpace: m2 } = s2, f3 = s2.transform ?? new d(), g2 = "local" === m2.type, b2 = e.spatialReference, y2 = b2.isGeographic, R2 = T(b2, p), h2 = K(p, b2) && J(p, b2);
    ...
    if (i2[t2.originX] = I3.x, i2[t2.originY] = I3.y, i2[t2.originZ] = I3.z ?? 0, null != f3) {
      const { translation: e2, scale: r2, rotation: s3 } = f3, o = g2 ? 1 : ce(p) / ce(b2);
      i2[t2.translationX] = e2[0] * o, i2[t2.translationY] = e2[2] * o, i2[t2.translationZ] = -e2[1] * o, i2[t2.scaleX] = r2[0], i2[t2.scaleY] = r2[2], i2[t2.scaleZ] = r2[1], i2[t2.rotationX] = s3[0], i2[t2.rotationY] = s3[2], i2[t2.rotationZ] = -s3[1], i2[t2.rotationDeg] = s3[3];
    }
    return { attributes: i2 };
  }
  return null == s2 ? { attributes: i2 } : "mesh" === s2.type || "extent" === s2.type ? null : { geometry: s2.toJSON(), attributes: i2 };
}
async function f2(e, t) {
  const r = await Promise.all((t.addAttachments ?? []).map((t2) => g(e, t2))), a = await Promise.all((t.updateAttachments ?? []).map((t2) => g(e, t2))), s2 = t.deleteAttachments ?? [];
  return r.length || a.length || s2.length ? { adds: r, updates: a, deletes: [...s2] } : null;
}
...
function h(t, r) {
  return new _({ attributes: t.attributes, geometry: f({ ...t.geometry, spatialReference: r }) });
}
function I2(e, t) {
...
//# sourceMappingURL=chunk-7AQ2PGF7.js.map
