...
var t4 = class {
  constructor(t5, r4, s5) {
    this._createTexture = t5, this._parametersKey = r4, this._repository = /* @__PURE__ */ new Map(), this._orphanCache = s5.newCache(`procedural-texture-repository:${e()}`, (e8) => e8.dispose());
  }
  destroy() {
    for (const { texture: e8 } of this._repository.values()) e8.dispose();
    this._repository.clear(), this._orphanCache.destroy();
  }
  swap(e8, t5 = null) {
    const r4 = this._acquire(e8);
    return this.release(t5), r4;
  }
  release(e8) {
    if (null == e8) return;
    const t5 = this._parametersKey(e8), r4 = this._repository.get(t5);
    if (r4 && (r4.refCount--, 0 === r4.refCount)) {
      this._repository.delete(t5);
      const { texture: e9 } = r4;
      this._orphanCache.put(t5, e9);
    }
  }
  _acquire(e8) {
    if (null == e8) return null;
    const t5 = this._parametersKey(e8), s5 = this._repository.get(t5);
    if (s5) return s5.refCount++, s5.texture;
    const o2 = this._orphanCache.pop(t5) ?? this._createTexture(e8), i4 = new r3(o2);
    return this._repository.set(t5, i4), o2;
  }
};
var r3 = class {
  constructor(e8) {
    this.texture = e8, this.refCount = 1;
  }
};
...
function i3(i4, u2) {
  return new t4((t5) => {
    const { data: u3, textureSize: l2 } = a2(t5), p2 = new h(l2, 1);
    return p2.dataType = N.FLOAT, p2.pixelFormat = 6403, p2.internalFormat = E.R16F, p2.wrapMode = 10497, new A(i4, p2, u3);
  }, (t5) => `${t5.pattern.join(",")}-r${t5.pixelRatio}`, u2);
}
function a2(t5) {
  const e8 = u(t5), r4 = 1 / t5.pixelRatio, o2 = l(t5), n5 = [];
  let i4 = 1;
  for (const u2 of e8) {
    for (let t6 = 0; t6 < u2; t6++) {
      const e9 = i4 * (Math.min(t6, u2 - 1 - t6) + 0.5) * r4;
      n5.push(e9);
    }
    i4 = -i4;
  }
  const a3 = Math.round(e8[0] / 2);
  return { data: new Float32Array([...n5.slice(a3), ...n5.slice(0, a3)]), textureSize: o2 };
}
function u(t5) {
...
return worldToScreenPerDistanceRatio / segmentDistanceToCamera;
  ...
  `), h2.uniforms.add(new e4("stipplePatternTexture", (e8) => e8.stippleTexture), new r2("stipplePatternPixelSizeInv", (e8) => 1 / f3(e8))), c3.stippleOffColorEnabled && h2.uniforms.add(new e3("stippleOffColor", (e8) => s3(e8.stippleOffColor))), p2.include(d3), h2.code.add(t`float getStippleSDF(out bool isClamped) {
...
function m2(t5, m3) {
  ...
  const { attributes: r4, varyings: d4, vertex: l2, fragment: T } = t5;
  r4.add("timeStamps", "vec4"), d4.add("vTimeStamp", "float"), d4.add("vFirstTime", "float"), d4.add("vLastTime", "float"), d4.add("vTransitionType", "float"), l2.main.add(t`vTimeStamp = timeStamps.x;
...
vTransitionType = timeStamps.w;`);
  ...
  3 === v && T.constants.add("decayRate", "float", 2.3), T.code.add(t`
    float getTrailOpacity(float x) {
      ${s4(v)}
    }`), T.uniforms.add(new r2("timeElapsed", (e8) => e8.timeElapsed), new r2("trailLength", (e8) => e8.trailLength), new r2("speed", (e8) => e8.animationSpeed), new e3("timingOptions", (t6) => s(n4, t6.startTime, t6.endTime, t6.fadeInTime, t6.fadeOutTime))), T.code.add(t`float fadeIn(float x) {
...
vec2(vStippleDistanceLimits.x, 1e34);`)), R.main.add(t`
    ...
    }`), z2.fragment.include(p, j2), z2.include(n3, j2), F.include(e6), F.main.add(t`discardBySlice(vpos);
discardByTerrainDepth();`), z2.include(d3), F.main.add(t`
  ...
  `), $ ? F.main.add(t`vec4 finalColor = vec4(1.0, 0.0, 1.0, 1.0);`) : (J && F.main.add(t`
      ...
      `) : F.main.add(t`float stippleAlpha = getStippleAlpha(lineWidth);`), 10 !== T && F.main.add(t`discardByStippleAlpha(stippleAlpha, ${t.float(o)});`), z2.include(d3), F.uniforms.add(new e3("intrinsicColor", (e8) => e8.color)).main.add(t`vec4 color = intrinsicColor * vColor;
color.a = noPerspectiveRead(color.a);`), _ && F.uniforms.add(new e3("innerColor", (e8) => e8.innerColor ?? e8.color), new r2("innerWidth", (e8, i4) => e8.innerWidth * i4.camera.pixelRatio)).main.add(t`float distToInner = abs(lineDistance) - innerWidth;
...
//# sourceMappingURL=chunk-Q6VSB54T.js.map
