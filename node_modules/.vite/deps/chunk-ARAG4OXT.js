...
var le = (_a = class extends l2(s5(u)) {
  ...
  async queryAttachments(e4, r4) {
    ...
    if (!t2?.operations?.supportsQueryAttachments) {
      if (i7?.length > 0 || a4?.length > 0 || n5?.length > 0 || l6 || p4 || u4) throw new s("queryAttachments:option-not-supported", "when 'capabilities.operations.supportsQueryAttachments' is false, only objectIds is supported", e4);
    }
    if (!(s9?.length || a4?.length || u4)) throw new s("queryAttachments:invalid-query", "'objectIds', 'globalIds', or 'where' are required to perform attachment query", e4);
  ...
  _handleSublayersChange(e4, r4) {
    r4 && (r4.forEach((e5) => {
      ...
      (this.layer?.capabilities?.exportMap?.supportsSublayersChanges ?? 1) || (i.getLogger(this).error(new s("sublayer:sublayers-non-modifiable", "Sublayer can't be added, moved, or removed from the layer's sublayers", { sublayer: this, layer: this.layer })), e5.preventDefault());
    })]));
  }
  _logLockedError(e4, r4) {
    const { layer: t2, declaredClass: i7 } = this;
    i.getLogger(i7).error(new s("sublayer:locked", `Property '${String(e4)}' can't be changed on Sublayer from the layer '${t2?.id}'`, { reason: r4, sublayer: this, layer: t2 }));
  }
  _getLayerDomain(e4) {
...
//# sourceMappingURL=chunk-ARAG4OXT.js.map
