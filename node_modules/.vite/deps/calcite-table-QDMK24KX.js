...
var _Pagination = class _Pagination extends LitElement {
  constructor() {
    super(...arguments);
...
_Pagination.properties = { isXXSmall: [16, {}, { state: true }], lastStartItem: [16, {}, { state: true }], maxItems: [16, {}, { state: true }], totalPages: [16, {}, { state: true }], groupSeparator: [7, {}, { reflect: true, type: Boolean }], messageOverrides: [0, {}, { attribute: false }], numberingSystem: 1, pageSize: [11, {}, { reflect: true, type: Number }], scale: [3, {}, { reflect: true }], startItem: [11, {}, { reflect: true, type: Number }], totalItems: [11, {}, { reflect: true, type: Number }] };
_Pagination.shadowRootOptions = { mode: "open", delegatesFocus: true };
_Pagination.styles = styles;
...
var _Table = class _Table extends LitElement {
  ...
  renderSelectionArea() {
    ...
    return html`<div class=${safeClassMap(CSS2.selectionArea)}><calcite-chip class=${safeClassMap({
      [CSS2.selectionCountChip]: true,
      [CSS2.selectionChipActive]: this.selectedCount > 0
    })} .kind=${this.selectedCount > 0 ? "brand" : "neutral"} .label=${selectionText} .scale=${this.scale} .value=${selectionText}>${selectionText}</calcite-chip>${outOfViewCount > 0 && html`<calcite-chip class=${safeClassMap(CSS2.selectionOutOfViewChip)} .icon=${ICONS2.hideEmpty} .label=${outOfView} .scale=${this.scale} title=${outOfView ?? nothing} .value=${outOfView}>${localizedOutOfView}</calcite-chip>` || ""}${this.selectedCount > 0 && html`<calcite-button class=${safeClassMap(CSS2.dismissButton)} .iconStart=${ICONS2.clear} kind=neutral @click=${this.handleDeselectAllRows} round .scale=${this.scale} .title=${`${this.messages.clear} ${selectionText} ${this.messages.row}`}>${this.messages.clear}</calcite-button>` || ""}<div class=${safeClassMap(CSS2.selectionActions)}><slot name=${SLOTS.selectionActions}></slot></div></div>`;
  }
  renderPaginationArea() {
    return html`<div class=${safeClassMap(CSS2.paginationArea)}><calcite-pagination .groupSeparator=${this.groupSeparator} .numberingSystem=${this.numberingSystem} @calcitePaginationChange=${this.handlePaginationChange} .pageSize=${this.pageSize} .scale=${this.scale} start-item=1 .totalItems=${this.bodyRows?.length} ${ref(this.paginationEl)}></calcite-pagination></div>`;
  }
  renderTHead() {
  ...
  render() {
    return html`<div class=${safeClassMap(CSS2.container)}>${this.selectionMode !== "none" && this.selectionDisplay !== "none" && this.renderSelectionArea() || ""}<div class=${safeClassMap({
      [CSS2.bordered]: this.bordered,
      [CSS2.striped]: this.striped,
    ...
    this.selectionMode === "multiple" ? "true" : null} .ariaRowCount=${this.allRows?.length} class=${safeClassMap({ [CSS2.tableFixed]: this.layout === "fixed" })} .role=${this.interactionMode === "interactive" ? "grid" : "table"} ${ref((el) => {
      if (!el) {
        return;
      }
      render(html`<caption class=${safeClassMap(CSS2.assistiveText)}>${this.caption}</caption>${this.renderTHead()}${this.renderTBody()}${this.renderTFoot()}`, el);
...
_Table.styles = styles2;
var Table = _Table;
customElement("calcite-table", Table);
export {
  Table
};
//# sourceMappingURL=calcite-table-QDMK24KX.js.map
