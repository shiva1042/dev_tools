...
var e = 8;
function o(h2, i2) {
  const { format: e2, quality: o2, rotation: n2, disableDecorations: a2 } = h2 || {}, r3 = b(h2, i2.padding), l2 = p(h2, { width: i2.width - r3.left - r3.right, height: i2.height - r3.top - r3.bottom }), { width: g2, height: f2 } = m(h2, l2), u2 = j(e2), d2 = R[u2];
  return { format: u2, quality: r(null != o2 ? o2 : d2, 0, 100), area: l2, width: g2, height: f2, rotation: n2, disableDecorations: !!a2, ignoreBackground: !!h2?.ignoreBackground, ignorePadding: !!h2?.ignorePadding };
}
function n(t, h2) {
  const i2 = o(t, h2), e2 = i2.area, n2 = i2.width / e2.width, a2 = b(i2, h2.padding), r3 = a2.left + a2.right, l2 = a2.top + a2.bottom, g2 = h2.width - r3, f2 = h2.height - l2, u2 = Math.floor(g2 * n2 + r3), d2 = Math.floor(f2 * n2 + l2), c2 = t?.layers ?? [], s3 = i2.ignoreBackground, w2 = i2.ignorePadding;
  return { framebufferWidth: u2, framebufferHeight: d2, region: { x: Math.floor(e2.x * n2) + a2.left, y: Math.floor(e2.y * n2) + a2.top, width: i2.width, height: i2.height }, format: i2.format, quality: i2.quality, rotation: i2.rotation, pixelRatio: n2, layers: c2, disableDecorations: i2.disableDecorations, ignoreBackground: s3, ignorePadding: w2, olidColor: false };
}
function a(t, h2, i2) {
  const { ctx: e2, canvas: o2 } = l(t, i2), n2 = e2.getImageData(0, 0, t.width, t.height), a2 = d(o2, h2);
...
function f() {
  return null == u && (u = document.createElement("canvas")), u;
}
var u = null;
function d(t, h2) {
...
function w(t, h2, i2, e2 = 0, o2 = 0, n2 = t.width - e2, a2 = t.height - o2, r3 = false) {
  const { data: l2 } = t, { width: g2, height: f2, data: u2 } = h2, d2 = n2 / g2, c2 = a2 / f2, s3 = Math.ceil(d2 / 2), w2 = Math.ceil(c2 / 2), M2 = t.width;
  for (let m2 = 0; m2 < f2; m2++) for (let t2 = 0; t2 < g2; t2++) {
    ...
    for (let r4 = Math.floor(m2 * c2); r4 < (m2 + 1) * c2; r4++) {
      const h4 = Math.abs(j2 - (r4 + 0.5)) / w2, g3 = (t2 + 0.5) * d2, f3 = h4 * h4;
      for (let u3 = Math.floor(t2 * d2); u3 < (t2 + 1) * d2; u3++) {
        const t3 = Math.abs(g3 - (u3 + 0.5)) / s3, h5 = Math.sqrt(f3 + t3 * t3);
        if (h5 >= 1) continue;
        let d3 = 2 * h5 * h5 * h5 - 3 * h5 * h5 + 1;
        const c3 = 4 * (e2 + u3 + (o2 + r4) * M2);
        b2 += d3 * l2[c3 + 3], a3 += d3, !i2 && l2[c3 + 3] < 255 && (d3 = d3 * l2[c3 + 3] / 255), p2 += d3 * l2[c3], x2 += d3 * l2[c3 + 1], y2 += d3 * l2[c3 + 2], n3 += d3;
      }
    }
    u2[h3] = p2 / n3, u2[h3 + 1] = x2 / n3, u2[h3 + 2] = y2 / n3, u2[h3 + 3] = b2 / a3;
  }
  return h2;
}
function M(t, o2, n2) {
  if (!o2) return t;
  const { framebufferWidth: a2, framebufferHeight: r3, pixelRatio: l2, region: g2 } = t, f2 = b(t, n2), u2 = f2.left + f2.right, d2 = f2.top + f2.bottom, c2 = a2 - u2, s3 = r3 - d2, w2 = Math.min(e, Math.min((h - u2) / c2, (h - d2) / s3));
  return w2 < i ? t : { ...t, framebufferWidth: Math.round(c2 * w2) + u2, framebufferHeight: Math.round(s3 * w2) + d2, pixelRatio: l2 * w2, resample: { region: { x: Math.round((g2.x - f2.left) * w2) + f2.left, y: Math.round((g2.y - f2.top) * w2) + f2.top, width: Math.round(g2.width * w2), height: Math.round(g2.height * w2) }, width: a2, height: r3 } };
}
function m(t, h2) {
  if (!t) return h2;
...
//# sourceMappingURL=chunk-CKFRCL5U.js.map
