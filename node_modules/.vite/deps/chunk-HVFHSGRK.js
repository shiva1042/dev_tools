...
function a6(t4) {
  ...
  if ("string" == typeof t4) {
    const c10 = u.fromString(t4);
    if (!c10) return n4().errorOnce(new s2("mapview:mesh-processing", "Unable to parse string into color", { color: t4 })), [0, 0, 0, 0];
    o16 = c10.toArray();
  } else o16 = t4;
...
var f6 = { createComputedParams: (t4) => t4, optionalAttributes: { zoomRange: { type: R.SHORT, count: 2, packPrecisionFactor: st, pack: ({ scaleInfo: t4 }, { tileInfo: e9 }) => s8(t4, e9) } }, attributes: { id: { type: R.UNSIGNED_BYTE, count: 3, pack: "id" }, pos: { type: R.SHORT, count: 2, pack: "position", packPrecisionFactor: 10 }, bitset: { type: R.UNSIGNED_BYTE, count: 1 }, color: { type: R.UNSIGNED_BYTE, count: 4, normalized: true, pack: ({ color: t4 }) => a6(t4) }, offset: { type: R.BYTE, count: 2, packPrecisionFactor: 16, packTessellation: ({ extrusionOffsetX: t4, extrusionOffsetY: e9 }) => [g2(t4, 16), g2(e9, 16)] }, normal: { type: R.BYTE, count: 2, packPrecisionFactor: 16, packTessellation: ({ normalX: t4, normalY: e9 }) => [g2(t4, 16), g2(e9, 16)] }, halfWidth: { type: R.HALF_FLOAT, count: 1, pack: ({ width: e9 }) => u2(0.5 * e9) }, referenceHalfWidth: { type: R.HALF_FLOAT, count: 1, pack: ({ referenceWidth: e9 }) => u2(0.5 * e9) } } };
var _2 = class {
  constructor() {
...
var T3 = class extends a7 {
  ...
  _writeVertices(e9, s19, r12) {
    const { _currentWrite: o16, _tessellationOptions: n12, evaluatedMeshParams: a14 } = this, { width: c9, capType: m7, joinType: u10, miterLimit: p8, hasSizeVV: d5 } = a14, f9 = u2(0.5 * c9);
...
function p5(e9) {
  const { sprite: o16, aspectRatio: s19, scaleProportionally: r12 } = e9, i12 = u2(e9.height), c9 = i12 > 0 ? i12 : o16.height;
  let a14 = i12 * s19;
...
var l11 = { createComputedParams: (t4) => t4, optionalAttributes: s14.optionalAttributes, attributes: { ...s14.attributes, bitset: { count: 1, type: R.UNSIGNED_BYTE, pack: n9 }, width: { count: 1, type: R.HALF_FLOAT, pack: (t4) => p5(t4).width }, height: { count: 1, type: R.HALF_FLOAT, pack: (t4) => p5(t4).height }, offset: { count: 2, type: R.HALF_FLOAT, pack: ({ offsetX: e9, offsetY: o16 }) => [u2(e9), -u2(o16)] }, scale: { count: 2, type: R.UNSIGNED_BYTE, packPrecisionFactor: 16, pack: ({ scaleX: t4, scaleY: e9 }) => [t4, e9] }, angle: { count: 1, type: R.UNSIGNED_BYTE, pack: ({ angle: t4 }) => l4(t4) } } };
...
var d4 = l11;
var h7 = m4;
var m5 = { createComputedParams: (e9) => e9, optionalAttributes: d4.optionalAttributes, attributes: { ...d4.attributes, bitset: { type: R.UNSIGNED_BYTE, count: 1, pack: (e9) => n9(e9) }, aux1: { count: 1, type: R.HALF_FLOAT, pack: (e9) => p5(e9).width }, aux2: { count: 1, type: R.HALF_FLOAT, pack: (e9) => p5(e9).height }, aux3: { count: 2, type: R.HALF_FLOAT, pack: ({ offsetX: t4, offsetY: s19 }) => [u2(t4), u2(s19)] }, aux4: { count: 2, type: R.UNSIGNED_BYTE, pack: ({ scaleX: e9, scaleY: t4 }) => [e9 * J, t4 * J] } } };
var x4 = { createComputedParams: (e9) => e9, optionalAttributes: d4.optionalAttributes, attributes: { ...d4.attributes, color: h7.attributes.color, bitset: { type: R.UNSIGNED_BYTE, count: 1, pack: (e9) => u3([[B, true]]) }, aux1: { count: 1, type: R.HALF_FLOAT, pack: (t4) => u2(0.5 * t4.width) }, aux2: { count: 1, type: R.HALF_FLOAT, pack: (t4) => u2(0.5 * t4.referenceWidth) }, aux3: { count: 2, type: R.HALF_FLOAT, packTessellation: ({ extrusionOffsetX: e9, extrusionOffsetY: t4 }) => [e9, t4] }, aux4: { count: 2, type: R.UNSIGNED_BYTE, packTessellation: ({ normalX: e9, normalY: t4 }) => [e9 * J + K, t4 * J + K] } } };
...
var c8 = class extends s15 {
  ...
  calculateRelativeSizeFromAbsolute() {
    ...
    return [u2(2 * t4) / (n12 - r12), u2(2 * t4) / (s19 - a14)];
  }
};
var x5 = class extends s15 {
  ...
  calculateRelativeSizeFromAbsolute() {
    const { _size: t4 } = this;
    return [u2(t4) / this.radius, 0];
  }
};
function l12(t4, e9) {
  if (null == t4) return null;
  const r12 = s(e9.angle), n12 = e9.gradientSize, s19 = e9.gradientSizeUnits;
...
var A3 = { createComputedParams: (t4) => t4, optionalAttributes: { zoomRange: { type: R.UNSIGNED_SHORT, count: 2, packPrecisionFactor: st, packTessellation: ({ minZoom: t4, maxZoom: e9 }) => [t4 || 0, e9 || _3] }, clipAngle: { type: R.UNSIGNED_BYTE, count: 1, packTessellation: ({ clipAngle: t4 }) => G2(t4 || 0) }, referenceSymbol: { type: R.BYTE, count: 4, packPrecisionFactor: 1, packTessellation: (t4, o16) => {
  ...
  return [u2(c9), -u2(l15), Math.round(u2(h9)), n12 + 1 << 2 | a14 + 1];
  ...
  const { bottomLeft: e9, bottomRight: o16, topLeft: i12, topRight: r12 } = t4;
  return [i12, r12, e9, o16];
} } }, color: { type: R.UNSIGNED_BYTE, count: 4, normalized: true, packTessellation: ({ color: t4 }) => t4 }, fontAndReferenceSize: { type: R.UNSIGNED_SHORT, count: 2, packPrecisionFactor: 4, packTessellation: ({ fontSize: t4 }, { referenceSize: o16 }) => [Math.round(u2(t4)), Math.round(u2(o16 ?? t4))] }, outlineColor: { type: R.UNSIGNED_BYTE, count: 4, normalized: true, pack: ({ outlineColor: t4 }) => a6(t4) }, haloColor: { type: R.UNSIGNED_BYTE, count: 4, normalized: true, pack: ({ haloColor: t4 }) => a6(t4) }, outlineAndHaloSize: { type: R.UNSIGNED_SHORT, count: 2, packPrecisionFactor: 4, packTessellation: ({ outlineSize: t4, haloSize: o16 }) => [Math.round(u2(t4)), Math.round(u2(o16))] } } };
var E2 = class extends a7 {
  ...
  _writePlacedTextMarkers(t4, r12, s19, n12) {
    ...
    if (!a14) return;
    const c9 = -1, l15 = e8.getPlacement(a14, c9, this.evaluatedMeshParams.placement, u2(1), t4.id);
    if (!l15) return;
  ...
  _getShaping(o16) {
    ...
    const s19 = u2(i12.fontSize), n12 = u2(i12.offsetX), p8 = u2(i12.offsetY), m7 = r(u2(r12.lineWidth), It, Jt), d5 = l2 * r(r12.lineHeightRatio, 0.25, 4);
    return z(r12.glyphs, { scale: s19 / q, angle: i12.postAngle, xOffset: n12, yOffset: p8, horizontalAlignment: r12.horizontalAlignment, verticalAlignment: o16 || r12.verticalAlignment, maxLineWidth: m7, lineHeight: d5, decoration: r12.decoration, borderLineSizePx: u2(r12.boxBorderLineSize), hasBackground: !!r12.boxBackgroundColor, useCIMAngleBehavior: r12.useCIMAngleBehavior });
  }
  _writeGlyphs(t4, o16, i12, r12, s19, n12, a14, c9, l15 = true) {
    const h9 = this.evaluatedMeshParams, f9 = this._textMeshTransformProps, m7 = u2(f9.fontSize), u10 = f9.haloSize, g6 = f9.outlineSize, x6 = u2(f9.offsetX), y4 = u2(f9.offsetY), [S5, _4] = s8(h9.scaleInfo, this.getTileInfo());
...
} }, accumulatedDistance: { type: R.HALF_FLOAT, count: 1, packTessellation: ({ distance: t4, pathLength: e9, distanceOffset: i12 }) => (i12 + t4) / e9 }, gradientSize: { type: R.HALF_FLOAT, count: 1, pack: ({ gradientSize: e9, gradientSizeUnits: r12 }) => r12 === e2.Relative ? e9 / 100 : u2(e9) }, totalLength: { type: R.HALF_FLOAT, count: 1, packTessellation: ({ pathLength: t4 }) => t4 }, segmentDirection: { type: R.BYTE, count: 2, packPrecisionFactor: 16, packTessellation: ({ directionX: t4, directionY: e9 }) => [t4, e9] } } };
...
} }, accumulatedDistance: { type: R.UNSIGNED_SHORT, count: 1, packTessellation: ({ distance: t4 }) => t4 }, segmentDirection: { type: R.BYTE, count: 2, packPrecisionFactor: 16, packTessellation: ({ directionX: t4, directionY: e9 }) => [t4, e9] }, offsetAlongLine: { type: R.HALF_FLOAT, count: 1, pack: ({ offsetAlongLine: e9 }) => u2(e9) }, capType: { type: R.UNSIGNED_BYTE, count: 1, pack: ({ capType: t4 }) => {
  switch (t4) {
    case "Butt":
    case "butt":
    default:
...
var o14 = class _o {
  ...
  static fromSimpleMeshParams(e9) {
    ...
    return i12.rawWidth = u2(h9), i12.rawHeight = u2(r12), i12.angle = n12, i12.alignment = a14, i12.outlineSize = u2(c9), i12.referenceSize = u2(f9), i12.overrideOutlineColor = m7, i12.offsetX = u2(e9.offsetX), i12.offsetY = u2(e9.offsetY), "simple" !== s19 || l15.sdf || (i12.rawWidth = l15.width, i12.rawHeight = l15.height), i12._computeSize(e9, false), i12;
  }
  static fromComplexMeshParams(i12) {
    ...
    s19.alignment = h9, s19.rawHeight = u2(n12), s19.rawWidth = s19.rawHeight * M3, s19.referenceSize = u2(w4), s19.sizeRatio = X2, s19.sdfDecodeCoeff = (C3.sdfDecodeCoeff ?? 1) * X2, s19.angle = l15, s19.rotateClockwise = Y, s19.anchorX = c9, s19.anchorY = f9, s19.offsetX = u2(p8), s19.offsetY = u2(g6), S5 && n12 && (C3.sdf ? s19.anchorX = c9 / (n12 * u10) : s19.anchorX = c9 / (n12 * M3), s19.anchorY = f9 / n12);
    const W = H && d5 ? n12 / d5 : 1;
    return s19.outlineSize = 0 === x6 || isNaN(x6) ? 0 : u2(x6) * W, s19.scaleSymbolsProportionally = H, s19.colorLocked = m7, s19._computeSize(i12, true), s19;
  }
...
var I2 = class extends a7 {
  ...
  _writePlacedMarkers(t4, r12, o16, s19, a14) {
    ...
    if (!n12) return;
    const m7 = -1, u10 = e8.getPlacement(n12, m7, this.evaluatedMeshParams.placement, u2(1), t4.id);
    if (!u10) return;
...
var o15 = { createComputedParams: (e9) => e9, optionalAttributes: {}, attributes: { pos: { type: R.SHORT, count: 2, packPrecisionFactor: 10, pack: "position" }, id: { type: R.UNSIGNED_BYTE, count: 3, pack: "id" }, bitset: { type: R.UNSIGNED_BYTE, count: 1, pack: (e9) => 0 }, offset: { type: R.SHORT, count: 2, packPrecisionFactor: 16, packAlternating: { count: 4, pack: ({ size: t4 }) => {
  const r12 = u2(t4), i12 = -r12 / 2, o16 = -r12 / 2;
  return [[i12, o16], [i12 + r12, o16], [i12, o16 + r12], [i12 + r12, o16 + r12]];
} } }, texCoords: { type: R.SHORT, count: 2, packPrecisionFactor: 4, packAlternating: { count: 4, pack: () => [[0, 1], [1, 1], [0, 0], [1, 0]] } }, size: { type: R.UNSIGNED_BYTE, count: 2, pack: ({ size: e9 }) => [e9, e9] }, referenceSize: { type: R.UNSIGNED_BYTE, count: 1, pack: ({ size: t4 }) => u2(t4) }, zoomRange: { type: R.UNSIGNED_BYTE, count: 2, pack: ({ scaleInfo: e9 }, { tileInfo: r12 }) => s8(e9, r12) } } };
var s18 = class extends a7 {
  constructor() {
    super(...arguments), this.vertexSpec = o15;
  }
  _write(t4, r12) {
    const i12 = r12.getDisplayId(), o16 = this.evaluatedMeshParams.minPixelBuffer, s19 = Math.max(u2(this.evaluatedMeshParams.size), o16);
    let c9, n12;
...
//# sourceMappingURL=chunk-HVFHSGRK.js.map
