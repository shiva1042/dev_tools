...
var v = class extends l2(s2(b)) {
  ...
  get state() {
    const { _queryAbortController: t2, _queryFeatureCountAbortController: e2, _queryPageAbortController: o3, canQuery: r2, _loaded: s4, canLoad: i, source: a2 } = this;
    return e2 || i && !s4 ? "loading" : t2 || o3 ? "querying" : !r2 || "popup" === a2 && 0 === this.featureCount ? "disabled" : "ready";
  }
  get utilityNetwork() {
  ...
  async _findLayersBySourceId(t2) {
    ...
    await e2?.load();
    const s4 = e2.getLayerIdBySourceId(t2), i = o3.allLayers.filter(r2), a2 = o3.allTables.filter(r2), n = i.concat(a2).toArray();
  ...
  async _queryLayer(t2, e2, o3, r2, s4) {
    const i = this._getFeatureQueryWhereClause(t2, e2, o3, r2), a2 = new b2({ where: i, outFields: ["*"], cacheHint: this.supportsCacheHint }), n = g.fromJSON(await r(t2, a2, s4));
    return n.features.forEach((e3) => {
      e3.layer = e3.sourceLayer = d(t2) ? t2.findSublayerForFeature(e3) : t2;
    }), n.features;
  }
  async _createAssociationFeatureObjects(t2, e2, o3, r2, s4, i) {
    if (0 === t2.length) return [];
    const a2 = /* @__PURE__ */ new Map();
    for (const [c, l3] of e2) {
      const t3 = await this._findLayersBySourceId(c);
      for (const e3 of t3) {
        (await this._queryLayer(e3, l3, r2, s4, i)).forEach((t4) => {
    ...
    return await Promise.all(t2.toArray().map(async (t3) => {
      const { fromNetworkElement: e3, toNetworkElement: r3 } = t3, s5 = e3.globalId === o3 ? r3 : e3, i2 = a2.get(s5.globalId) ?? [];
      await Promise.all(i2.map(async (e4) => {
        const o4 = this.utilityNetwork?.getTerminalById(s5?.terminalId)?.name, r4 = e4.sourceLayer && "getFeatureTitle" in e4.sourceLayer && await e4.sourceLayer.getFeatureTitle(e4) || t(e4);
  ...
  _parseFeatureObjects(t2, e2) {
    ...
    for (const [r2, s4] of o3) this._sortFeatureObjectsByTitle(s4), e2.set(r2, s4);
  }
  ...
  async _queryAssociations(t2) {
    const { layer: e2, globalId: o3, associationTypes: r2, utilityNetwork: s4, canQuery: i } = this;
    if (await Promise.allSettled([e2?.load(), s4?.load()]), this._clearAssociations(), !(i && e2 && r2 && s4 && o3)) return;
    const a2 = m2(e2) ? e2.parent : e2, n = new s3({ globalId: o3, networkSourceId: s4.getSourceIdByLayerId(a2.layerId) }), c = /* @__PURE__ */ new Set();
    r2.forEach((t3) => {
      switch (t3.type) {
        case "attachment":
        case "structure":
    ...
    const l3 = await s4?.queryAssociations({ elements: [n], types: Array.from(c) }, { signal: t2?.signal }), u = /* @__PURE__ */ new Map(), y2 = /* @__PURE__ */ new Map();
    r2.forEach((t3) => {
    ...
    const d2 = r2.map(async (e3) => {
      const { associatedNetworkSourceId: o4, associatedAssetGroup: r3, associatedAssetType: s5 } = e3, i2 = u.get(e3.type), a3 = null != r3 ? await this._countAssociatedFeatures(o4, i2, r3, s5, t2) : i2.length;
      switch (e3.type) {
        case "attachment":
  ...
  async _countAssociatedFeatureCount(t2, e2, o3, r2, s4) {
  ...
  async _countAssociatedFeatures(t2, e2, o3, r2, s4) {
    if (0 === e2.length) return 0;
    const i = (await this._findLayersBySourceId(t2)).map(async (t3) => this._countAssociatedFeatureCount(t3, e2, o3, r2, s4));
    return (await Promise.all(i)).reduce((t3, e3) => t3 + e3, 0);
  }
  async _queryAssociatedFeatures(t2, e2) {
    const { layer: o3, globalId: r2, associationTypes: s4, utilityNetwork: i, canQuery: a2, associationFeatures: n } = this;
    if (await Promise.allSettled([o3?.load(), i?.load()]), !(a2 && o3 && s4 && i)) return;
    const c = this._getAssociationsByType(t2.type), { associatedAssetGroup: l3, associatedAssetType: u } = t2, y2 = /* @__PURE__ */ new Map();
    c.forEach((t3) => {
      const { fromNetworkElement: e3, toNetworkElement: o4 } = t3, { networkSourceId: s5, elementGlobalId: i2 } = e3.globalId === r2 ? { networkSourceId: o4.networkSourceId, elementGlobalId: o4.globalId } : { networkSourceId: e3.networkSourceId, elementGlobalId: e3.globalId }, a3 = y2.get(s5) || [];
      a3.push(i2), y2.set(s5, a3);
    });
  ...
  _shouldDiscardNetworkElement(t2, e2, o3) {
    if (!t2) return false;
    const { networkSourceIdsInUse: r2 } = this, { networkSourceId: s4 } = t2, i = o3.get(e2)?.associatedNetworkSourceId, a2 = r2.has(s4);
    return null != i && i !== s4 || !a2;
  }
  _getFeatureQueryWhereClause(t2, e2, o3, r2) {
    const s4 = t2.globalIdField, i = t2.fieldsIndex.get(o2.assetGroup), a2 = t2.fieldsIndex.get(o2.assetType), n = null != o3, c = null != r2;
    return [s4 ? o(s4, e2) : null, n ? `(${i?.name} = ${o3})` : null, n && c ? `(${a2?.name} = ${r2})` : null].filter(Boolean).join(" AND ");
  }
};
...
//# sourceMappingURL=chunk-PKN5SRD2.js.map
