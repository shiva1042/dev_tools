...
function _(e) {
  if (!e) return null;
  const t = e.vcsWkid || e.latestVcsWkid;
...
function E2(f2, S2 = false) {
  let _2 = new f({ wkid: 4326, vcsWkid: 115700 }), R3 = f2.root?.boundingVolume, E3 = false, b = f2?.root?.transform;
  if (S2 && (f2.extensions?.ESRI_crs?.wkid || f2.extensions?.ESRI_crs?.latestWkid || f2.extensions?.ESRI_crs?.wkt) && (f2.root?.extensions?.ESRI_crs?.boundingVolume?.box || f2.root?.extensions?.ESRI_crs?.boundingVolume?.sphere)) _2 = new f(f2.extensions?.ESRI_crs), R3 = f2.root?.extensions?.ESRI_crs?.boundingVolume, b = f2.root?.transform, E3 = true;
  else if (!R3) return new z(-180, -90, 180, 90, _2);
  if (R3.box) {
    const e = R3?.box;
    if (e[3] > d2 && e[7] > d2 && e[11] > d2) return new z({ ...p2, spatialReference: _2 });
  }
  const w = n();
  if (!E3 && R3.region && u2(R3.region, 6)) {
    const t = R3.region, r2 = M(t[0]), n2 = M(t[1]), o2 = t[4], i = M(t[2]), s2 = M(t[3]), m2 = t[5];
    return new z({ xmin: r2, ymin: n2, zmin: o2, xmax: i, ymax: s2, zmax: m2, spatialReference: _2 });
  }
  if (R3.sphere && u2(R3.sphere, 4)) {
    ...
    const S3 = n(), y = n();
    return l(S3, d3, p3), d(y, d3, p3), new z({ xmin: S3[0], ymin: S3[1], zmin: S3[2], xmax: y[0], ymax: y[1], zmax: y[2], spatialReference: _2 });
  }
  if (R3.box && u2(R3.box, 12)) {
    ...
    return new z({ xmin: w2[0], ymin: w2[1], zmin: w2[2], xmax: S3[0], ymax: S3[1], zmax: S3[2], spatialReference: _2 });
  }
  return new z(-180, -90, 180, 90, _2);
}
...
//# sourceMappingURL=chunk-I6HWFDQ7.js.map
