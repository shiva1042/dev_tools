...
// node_modules/@arcgis/core/views/3d/webgl-engine/core/shaderLibrary/util/ComputeCovariance.glsl.js
function c2(c3) {
  c3.code.add(t`void computeCovariance3D(in mat3 rotation, in vec3 scale, out vec3 covarianceA, out vec3 covarianceB) {
...
// node_modules/@arcgis/core/views/3d/webgl-engine/core/shaderLibrary/util/GaussianSplatUnpacking.glsl.js
function e5(e6) {
  e6.code.add(t`vec4 unpackColor(uvec4 packedGaussian) {
vec4 color;
...
return color / 255.0;
}`), e6.code.add(t`vec3 unpackScale(uvec4 packedGaussian) {
...
}`), e6.code.add(t`const uint MASK_9_BITS = 0x1FFu;
...
return quaternion;
}`), e6.code.add(t`vec3 unpackTileOriginRelativePosition(uvec4 packedGaussian) {
...
}`), e6.code.add(t`vec3 unpackCameraRelativeGaussianPosition(uvec4 packedHeader, highp vec3 position, vec3 cameraPosition, vec3 cameraPos8k, vec3 cameraDelta) {
...
// node_modules/@arcgis/core/views/3d/webgl-engine/core/shaderLibrary/util/QuaternionToRotationMatrix.glsl.js
function y(y3) {
  y3.code.add(t`mat3 quaternionToRotationMatrix(vec4 q) {
float x2 = q.x + q.x;
float y2 = q.y + q.y;
float z2 = q.z + q.z;
float xx = x2 * q.x;
...
// node_modules/@arcgis/core/chunks/GaussianSplat.glsl.js
var x = class extends c {
  constructor() {
    super(...arguments), this.totalGaussians = -1, this.focalLength = -1, this.minSplatRadius = -1, this.minSplatOpacity = -1, this.tanFov = l, this.cameraDelta = a, this.cameraPos8k = a;
  }
};
function w(e6) {
  const n2 = new s();
  n2.varyings.add("vColor", "vec4"), n2.varyings.add("conicOpacity", "vec4"), n2.varyings.add("depth", "float"), n2.varyings.add("gaussianCenterScreenPos", "vec2"), n2.varyings.add("fragScreenPos", "vec2"), n2.outputs.add("fragColor", "vec4", 0), n2.vertex.uniforms.add(new e4("splatOrderTexture", (e7) => e7.splatOrder), new e4("splatAtlasTexture", (e7) => e7.splatAtlas), new o("numSplats", (e7) => e7.totalGaussians), new r("focalLength", (e7) => e7.focalLength), new r("minSplatRadius", (e7) => e7.minSplatRadius), new r("minSplatOpacity", (e7) => e7.minSplatOpacity), new e3("tanFov", (e7) => e7.tanFov), new e("cameraDelta", (e7) => e7.cameraDelta), new e("cameraPos8k", (e7) => e7.cameraPos8k), new o2("fullWidth", ({ camera: e7 }) => e7.viewport[2]), new o2("fullHeight", ({ camera: e7 }) => e7.viewport[3]), new t2("proj", (e7) => e7.camera.projectionMatrix), new t2("view", (e7) => e7.camera.viewMatrix), new e2("cameraPosition", (e7) => e7.camera.eye)), n2.vertex.include(e5), n2.vertex.include(y), n2.vertex.include(c2), n2.include(i, e6), n2.vertex.code.add(t`float ndcToPixel(float ndcCoord, float screenSize) {
return ((ndcCoord + 1.0) * screenSize - 1.0) * 0.5;
  ...
  const g = e6.depthPass;
  return n2.fragment.main.add(`
    ...
    ${n(g, "fragColor = vec4(depth, 0, 0, alpha);", "fragColor = vec4(vColor.rgb * alpha, alpha);")}
  `), n2;
}
var y2 = Object.freeze(Object.defineProperty({ __proto__: null, GaussianSplatPassParameters: x, build: w }, Symbol.toStringTag, { value: "Module" }));
...
//# sourceMappingURL=chunk-AKSJQDPT.js.map
