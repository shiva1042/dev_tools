...
var l = [[0.4124, 0.3576, 0.1805], [0.2126, 0.7152, 0.0722], [0.0193, 0.1192, 0.9505]];
...
function b(n, t) {
  ...
  const u2 = n.length, c = n[0].length;
  let i2 = 0;
  for (o = 0; o < u2; o++) {
...
function m(n) {
  const t = [n.r / 255, n.g / 255, n.b / 255].map((n2) => n2 <= 0.04045 ? n2 / 12.92 : ((n2 + 0.055) / 1.055) ** 2.4), r3 = b(l, t);
...
function x(n) {
  ...
  let u2 = Math.atan2(o, r3);
  return u2 = u2 > 0 ? u2 : u2 + 2 * Math.PI, { l: t, c: e, h: u2 };
}
function k(n) {
...
function z(n) {
  const t = n.r, r3 = n.g, o = n.b, e = Math.max(t, r3, o), u2 = e - Math.min(t, r3, o);
  let c = e, i2 = 0 === u2 ? 0 : e === t ? (r3 - o) / u2 % 6 : e === r3 ? (o - t) / u2 + 2 : (t - r3) / u2 + 4, a3 = 0 === u2 ? 0 : u2 / c;
  return i2 < 0 && (i2 += 6), i2 *= 60, a3 *= 100, c *= 100 / 255, { h: i2, s: a3, v: c };
}
function A(n) {
  const t = (n.h + 360) % 360 / 60, r3 = n.s / 100, o = n.v / 100 * 255, e = o * r3, u2 = e * (1 - Math.abs(t % 2 - 1));
  let c;
  switch (Math.floor(t)) {
    case 0:
      c = { r: e, g: u2, b: 0 };
...
function R(t) {
  let { r: r3, g: o, b: e, a: u2 } = t;
  return u2 < 1 && (r3 = Math.round(u2 * r3 + 255 * (1 - u2)), o = Math.round(u2 * o + 255 * (1 - u2)), e = Math.round(u2 * e + 255 * (1 - u2))), new u({ r: r3, g: o, b: e });
}
function E(n, t) {
  const { r: r3, g: o, b: e } = t?.ignoreAlpha ? n : R(n);
  return 0.2126 * r3 + 0.7152 * o + 0.0722 * e;
}
function I(t, r3 = 225) {
  return E(t, { ignoreAlpha: true }) > r3 ? new u([0, 0, 0, t.a]) : new u([255, 255, 255, t.a]);
}
function P(n, t) {
  const r3 = C2(n);
  r3.l *= 1 - t;
...
function K(n, t) {
  return n === t || null != n && null != t && L(n, t);
}
function L2(n) {
  return r2(n[0], n[1], n[2], n.length > 3 ? n[3] : 1);
}
function N(n) {
  return n[0] ??= 0, n[1] ??= 0, n[2] ??= 0, n;
}
function O(n) {
  return n.length = 4, N(n), n[3] ??= 1, r(n[3], 0, 1), n;
}
function Q(t, r3) {
  const o = u.toUnitRGBA(t);
  return o[3] *= r3, o;
}
function S(r3, e, u2 = {}) {
  if (0 === r3.length || e <= 0) return [];
  if (1 === (r3 = r3.map((t) => "string" == typeof t ? new u(t) : t)).length || 1 === e) {
    const n = [], t = r3[0];
    for (let r4 = 0; r4 < e; r4++) n.push(t.clone());
    return n;
  }
  if (u2.shuffle && (r3 = G(a(r3), u2.seed)), r3.length >= e) {
...
function T(n, r3, o = {}) {
  const e = [], u2 = n.length - 1, c = Math.ceil((r3 - n.length) / u2);
  n: for (let t = 0; t < u2; t++) {
    const u3 = n[t], i2 = n[t + 1];
    for (let t2 = 1; t2 <= c; t2++) {
      const a3 = t2 / (c + 1);
      if (e.push(W(u3, i2, a3, o)), e.length + n.length === r3) break n;
    }
  }
...
function W(t, o, e, u2 = {}) {
  const c = t.r, i2 = t.g, a3 = t.b, s2 = o.r, f2 = o.g, h2 = o.b, l2 = Math.round(c + (s2 - c) * e), g2 = Math.round(i2 + (f2 - i2) * e), b2 = Math.round(a3 + (h2 - a3) * e);
  if (!u2.offset) return new u([l2, g2, b2]);
  const m2 = C(u2.seed);
  return new u([V(l2, m2), V(g2, m2), V(b2, m2)]);
}
...
//# sourceMappingURL=chunk-NT34C4FF.js.map
