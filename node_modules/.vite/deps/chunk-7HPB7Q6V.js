...
function r(t2, e2) {
  ...
  const r3 = e2.filter((t3) => "size" === t3.type).map((e3) => {
    const { maxSize: r4, minSize: o4 } = e3;
    return (n(r4, t2) + n(o4, t2)) / 2;
  });
  let o3 = 0;
  const s = r3.length;
  if (0 === s) return t2;
  for (let n2 = 0; n2 < s; n2++) o3 += r3[n2];
  const f = Math.floor(o3 / s);
  return Math.max(f, t2);
}
function o(e2) {
  const n2 = e2?.renderer, o3 = e2?.pointerType, s = "touch" === o3 ? 9 : 6;
  if (!n2) return s;
...
function r2(n2, r3, i2, a2 = new z()) {
  let o3 = 0;
  if ("2d" === i2.type) o3 = r3 * (i2.resolution ?? 0);
  else if ("3d" === i2.type) {
    const t2 = i2.overlayPixelSizeInMapUnits(n2), a3 = i2.basemapSpatialReference;
    o3 = null == a3 || a3.equals(i2.spatialReference) ? r3 * t2 : ae(a3) / ae(i2.spatialReference);
  }
  const s = n2.x - o3, l = n2.y - o3, c = n2.x + o3, m = n2.y + o3, { spatialReference: p } = i2;
  return a2.xmin = Math.min(s, c), a2.ymin = Math.min(l, m), a2.xmax = Math.max(s, c), a2.ymax = Math.max(l, m), a2.spatialReference = p, a2;
}
function i(e2, t2, i2) {
  const o3 = i2.toMap(e2);
  if (null == o3) return false;
  return r2(o3, o(), i2, a).intersects(t2);
}
var a = new z();
function o2() {
  a = new z();
...
//# sourceMappingURL=chunk-7HPB7Q6V.js.map
