...
function C2(e, t) {
  switch (h(e) && e.offset && (e.offset *= t), e.type) {
...
function u2(o) {
  const r = [];
  return y2(y(o), r), r.length ? new u(M(r[0])) : null;
}
function y2(e, t) {
  if (!e) return;
  let n;
  ...
  if (n?.symbolLayers) {
    for (const i of n.symbolLayers) if (!(i.colorLocked || s && (C(i) || m(i) && i.markerPlacement && h(i.markerPlacement)))) switch (i.type) {
      case "CIMPictureMarker":
      case "CIMPictureStroke":
      case "CIMPictureFill":
  ...
  if (i?.symbolLayers) for (const l of i.symbolLayers) {
    ...
    if (c) {
      if (n) {
        ...
        if ((C(l) || m(l) && l.markerPlacement && h(l.markerPlacement)) && "fill" === e3 || p(l) && "outline" === e3) continue;
      } else if (C(l) || m(l) && l.markerPlacement && h(l.markerPlacement)) continue;
    }
    const e2 = t.toArray();
    switch (l.type) {
      case "CIMPictureMarker":
...
//# sourceMappingURL=chunk-XNTUTP74.js.map
