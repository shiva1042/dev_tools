...
import {
  ...
  hasActiveChildren,
  isSingleLike
} from "./chunk-OOSM7N2S.js";
import {
  filter
} from "./chunk-Y3EPFBDI.js";
import {
  escapeRegExp
} from "./chunk-EF3BSNKC.js";
import {
  useCancelable
...
var _Combobox = class _Combobox extends LitElement {
  constructor() {
    super();
  ...
  clickHandler(event) {
    ...
    if (composedPath.some((node) => node.classList?.contains(CSS2.button))) {
      this.clearValue();
      event.preventDefault();
      return;
    }
  ...
  renderFloatingUIContainer() {
    ...
    return html`<div aria-hidden=true class=${safeClassMap(CSS.floatingUIContainer)} ${ref(setFloatingEl)}><div class=${safeClassMap(classes)} ${ref(setContainerEl)}><ul class=${safeClassMap({ [CSS.list]: true, [CSS.listHide]: !open })}>${this.selectAllEnabled && this.selectionMode !== "single" && this.selectionMode !== "single-persist" && html`<calcite-combobox-item class=${safeClassMap(CSS.selectAll)} .id=${`${this.guid}-select-all-enabled-interactive`} .indeterminate=${this.indeterminate} .label=${messages.selectAll} .scale=${scale} .selected=${this.allSelected} tabindex=-1 text-label=${messages.selectAll ?? nothing} value=select-all ${ref(this.selectAllComboboxItemRef)}></calcite-combobox-item>` || ""}<slot></slot>${this.noMatchesFound && (this.allowCustomValues ? html`<li aria-label=${label ?? nothing} class=${safeClassMap(CSS.noMatches)} @click=${this.customChipAddHandler} role=option tabindex=0>${highlightText({
      text: label,
...
//# sourceMappingURL=chunk-LBWZEQXL.js.map
