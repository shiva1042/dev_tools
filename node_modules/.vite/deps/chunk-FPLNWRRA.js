...
function l(i2) {
  i2.include(f), i2.fragment.uniforms.add(new t2("inverseViewMatrix", (i3, t5) => {
    const a2 = i(e(), t5.camera.viewMatrix, i3.localOrigin);
    return e2(a2, a2);
  })).code.add(t`vec4 reconstructLocalPosition(vec2 coord, float linearDepth) {
...
var x = class extends m {
  constructor() {
    super(...arguments), this.shadowMap = { depthTexture: null, nearFar: [1, 100], numActiveFaces: 1, atlasRegions: [[0, 0, 1, 1]] }, this.targetVector = [1, 0, 0], this.upVector = [0, 0, 1], this.fovs = [45, 45], this.headingAndTilt = [0, 0], this.observerOffset = [0, 0, 0], this.projectionMatrices = r.flat(), this.viewMatrices = r.flat(), this.volumeOffset = 0;
  }
};
function V() {
  const i2 = new s(), o4 = i2.fragment;
  return i2.include(o3), i2.include(l), o4.include(a), o4.include(t4), o4.uniforms.add(new e4("depthTexture", (e7) => e7.depth?.attachment), new t3("inverseProjectionMatrix", (e7) => e7.camera.inverseProjectionMatrix), new t3("inverseViewNormalMatrix", ({ camera: i3 }) => e2(b, i3.viewInverseTransposeMatrix)), new e3("viewshedObserverOffset", (e7) => e7.observerOffset), new e3("viewshedTargetVector", (e7) => e7.targetVector), new e3("viewshedUpVector", (e7) => e7.upVector), new e6("viewshedFOVs", (e7) => e7.fovs), new e6("viewshedHeadingAndTilt", (e7) => e7.headingAndTilt), new e6("viewshedNearFar", (e7) => e7.shadowMap.nearFar ?? [1, 100]), new r3("viewshedVolumeOffset", (e7) => e7.volumeOffset), new e5("viewshedShadowMap", (e7) => e7.shadowMap.depthTexture), new s2("viewshedProjectionMatrices", (e7) => e7.projectionMatrices, 6), new s2("viewshedViewMatrices", (e7) => e7.viewMatrices, 6), new o2("viewshedNumFaces", (e7) => e7.shadowMap.numActiveFaces), new r2("viewshedAtlasRegions", (e7) => e7.shadowMap.atlasRegions.flat(), 24), new e5("normalMap", (e7) => e7.normals)), o4.constants.add("visibleColor", "vec4", [0, 1, 0, 0.5]), o4.constants.add("occludedColor", "vec4", [1, 0, 0, 0.5]), o4.code.add(t`vec2 getViewshedUv(vec4 worldPosition, int face) {
...
}`), o4.main.add(t`float depth = depthFromTexture(depthTexture, uv);
...
var b = e();
var M = Object.freeze(Object.defineProperty({ __proto__: null, ViewshedPassParameters: x, build: V }, Symbol.toStringTag, { value: "Module" }));
...
//# sourceMappingURL=chunk-FPLNWRRA.js.map
