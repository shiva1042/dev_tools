...
function e3() {
  return a || (a = (async () => {
    const [a3, e6, r2] = await Promise.all([import("./arcadeUtils-3DWYAKBR.js"), import("./batchExec-D3TSFHK6.js"), import("./aiServices-4AIBDI6C.js")]);
...
var m2 = u(r, s3);
function d(e6) {
  return null != e6 && m2.has(e6.type);
}
function c2(e6) {
  return null != e6 && ("string" === e6.type || "esriFieldTypeString" === e6.type);
}
function p(e6, n, i2) {
  const t4 = i2 ?? e6?.domain;
  if (!t4) return null;
  switch (t4.type) {
    case "range": {
      const { min: i3, max: r2 } = y(e6, t4);
      if (null != i3 && +n < i3 || null != r2 && +n > r2) return "domain-validation-error::value-out-of-range";
      break;
    }
    case "coded-value":
    case "codedValue":
      if (null == t4.codedValues || t4.codedValues.every((e7) => null == e7 || e7.code !== n)) return "domain-validation-error::invalid-coded-value";
  }
  return null;
}
function y(e6, n) {
  ...
  return u5(e6) || o(e6) || a2(e6) ? { ...g(e6, r2, t4), isInteger: s4 } : { min: null != t4 && "number" == typeof t4 ? t4 : null, max: null != r2 && "number" == typeof r2 ? r2 : null, rawMin: t4, rawMax: r2, isInteger: s4 };
}
function f3(e6, n) {
  switch (n.type) {
    case "range": {
      const { min: i2, max: t4 } = y(e6, n);
      return u5(e6) || o(e6) || a2(e6) ? !!i2 && !!t4 : "number" == typeof i2 && Number.isFinite(i2) && "number" == typeof t4 && Number.isFinite(t4);
    }
    case "codedValue":
    case "coded-value": {
      const i2 = typeof n.codedValues[0].code;
      if ("string" === i2 && c2(e6) || "number" === i2 && d(e6)) return true;
    }
  }
  return false;
}
function g(e6, r2, l3) {
  return u5(e6) ? { min: e4(l3)?.toMillis(), max: e4(r2)?.toMillis(), rawMin: l3, rawMax: r2 } : o(e6) ? { min: H(l3)?.toMillis(), max: H(r2)?.toMillis(), rawMin: l3, rawMax: r2 } : a2(e6) ? { min: c(l3)?.toMillis(), max: c(r2)?.toMillis(), rawMin: l3, rawMax: r2 } : { max: null, min: null };
}
...
function e5(t4) {
  return t4.match(u6)?.[1]?.replaceAll("\\'", "'") ?? null;
}
var u6 = /^hash\(\$feature\['((\\'|[^'])+)'\]\) \* 8\.381e-8$/;
...
function c3(e6) {
  ...
  return e6.trim().replaceAll(d2, "_").replace(f4, "F$1") || null;
}
var p2 = ["field", "field2", "field3", "normalizationField", "rotationInfo.field", "proportionalSymbolInfo.field", "proportionalSymbolInfo.normalizationField", "colorInfo.field", "colorInfo.normalizationField"];
var m3 = ["field", "normalizationField"];
function y2(e6, n) {
  if (null != e6 && null != n) {
    for (const i2 of Array.isArray(e6) ? e6 : [e6]) if (g2(p2, i2, n), "visualVariables" in i2 && i2.visualVariables) for (const e7 of i2.visualVariables) g2(m3, e7, n);
  }
}
...
function $e(e6, n) {
  const i2 = y(e6, n);
  return i2 || (ge(e6) ? _e(e6.type) : void 0);
}
function _e(e6) {
  switch (e6) {
    case "esriFieldTypeSmallInteger":
...
//# sourceMappingURL=chunk-S2WGAJV4.js.map
