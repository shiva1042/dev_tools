...
import {
  f
} from "./chunk-HRKH3NSH.js";
import {
  g,
  v
} from "./chunk-55L2AQKW.js";
import {
  r as r3
...
var l3 = class extends m2 {
  constructor(r6, t7) {
    super(r6, t7, new t2(f, () => import("./SSAOBlur.glsl-P42AFB2D.js")), s);
  }
  initializePipeline() {
...
var l4 = class extends m2 {
  constructor(r6, t7) {
    super(r6, t7, new t2(g, () => import("./SSAO.glsl-ZZ4QOAZO.js")), s);
  }
  initializePipeline() {
    return T({ colorWrite: d });
  }
...
var j2 = class extends a2 {
  constructor(e4) {
    super(e4), this.consumes = { required: ["normals"] }, this.produces = j.SSAO, this.isEnabled = () => false, this._enableTime = n(0), this._passParameters = new t5(), this._drawParameters = new c2();
  }
  initialize() {
    const e4 = Uint8Array.from(atob(e3), (e5) => e5.charCodeAt(0)), r6 = new h(32);
    r6.wrapMode = 33071, r6.pixelFormat = 6407, r6.wrapMode = 10497, r6.hasMipmap = true, this._passParameters.noiseTexture = new A(this.renderingContext, r6, e4), this.techniques.precompile(l4), this.techniques.precompile(l3), this.addHandles(l(() => this.isEnabled(), () => this._enableTime = n(0)));
  }
  destroy() {
    this._passParameters.noiseTexture = r(this._passParameters.noiseTexture);
  }
  render(e4) {
    const t7 = e4.find(({ name: e5 }) => "normals" === e5), s2 = t7?.getTexture(), o6 = t7?.getTexture(n3);
    if (!s2 || !o6) return;
    const a5 = this.techniques.get(l4), p3 = this.techniques.get(l3);
    if (!a5.compiled || !p3.compiled) return this._enableTime = n(performance.now()), void this.requestRender(1);
    0 === this._enableTime && (this._enableTime = n(performance.now()));
    const l5 = this.renderingContext, d2 = this.view.qualitySettings.fadeDuration, f3 = this.bindParameters, T2 = f3.camera, _ = T2.relativeElevation, j3 = r2((o2 - _) / (o2 - e2), 0, 1), x = d2 > 0 ? Math.min(d2, performance.now() - this._enableTime) / d2 : 1, q = x * j3;
    this._passParameters.normalTexture = s2, this._passParameters.depthTexture = o6, this._passParameters.projScale = 1 / T2.computeScreenPixelSizeAtDist(1), this._passParameters.intensity = 4 * P / v(T2) ** 6 * q;
    const v3 = T2.fullViewport[2], y = T2.fullViewport[3], A2 = this.fboCache.acquire(v3, y, "ssao input", 2);
    l5.bindFramebuffer(A2.fbo), l5.setViewport(0, 0, v3, y), l5.bindTechnique(a5, f3, this._passParameters, this._drawParameters), l5.screen.draw();
...
function t6(t7, o6) {
  o6.receiveAmbientOcclusion ? (t7.uniforms.add(new e("ssaoTex", (e4) => e4.ssao?.getTexture())), t7.constants.add("blurSizePixelsInverse", "float", 1 / g2), t7.code.add(t`float evaluateAmbientOcclusionInverse() {
vec2 ssaoTextureSizeInverse = 1.0 / vec2(textureSize(ssaoTex, 0));
return texture(ssaoTex, gl_FragCoord.xy * blurSizePixelsInverse * ssaoTextureSizeInverse).r;
}
float evaluateAmbientOcclusion() {
return 1.0 - evaluateAmbientOcclusionInverse();
}`)) : t7.code.add(t`float evaluateAmbientOcclusionInverse() { return 1.0; }
...
function a4(a5) {
  a5.code.add(t`float mapChannel(float x, vec2 p) {
return (x < p.x) ? mix(0.0, p.y, x/p.x) : mix(p.y, 1.0, (x - p.x) / (1.0 - p.x) );
}`), a5.code.add(t`vec3 blackLevelSoftCompression(vec3 color, float averageAmbientRadiance) {
...
function f2(g3, f3) {
  ...
  L.include(t6, f3), 0 !== b && L.include(n5, f3), g3.include(m3, f3), L.include(t4), L.include(o4, f3);
  const R = !(2 === b && !N);
  switch (R && L.include(a4), L.code.add(t`
  ...
  `), a3(L), L.code.add(t`vec3 evaluateAdditionalLighting(float ambientOcclusion, vec3 vPosWorld) {
...
inputs.averageAmbientRadiance = ambientLightIrradianceComponent[1] * (1.0 + GROUND_REFLECTANCE);`), L.code.add(t`
    ...
    case 5:
    case 6:
      t3(L), a3(L), L.code.add(t`const float roughnessTerrain = 0.5;
...
//# sourceMappingURL=chunk-L5FZCSSN.js.map
